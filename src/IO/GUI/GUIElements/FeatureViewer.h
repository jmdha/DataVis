        ...
        if (DataManager::IsGeneratingFeatures() && !DataManager::HasFeatures())
            ...
            for (const auto &seriesSet : data)
                for (const auto &series : seriesSet.second)
                    classValues[seriesSet.first].push_back(feature.attribute->GenerateValue(series, feature.shapelet));

            std::vector<double> avgs;
            for (const auto &valueSet : classValues)
                avgs.push_back(std::accumulate(valueSet.second.begin(), valueSet.second.end(), (double) 0) / (double) valueSet.second.size());
